			...
			if(minigame.getType().equals("teamdm")){
				...
				if(team == 1){
					if(minigame.getRedTeam().size() < minigame.getBlueTeam().size() - 1){
						...
						team = 0;
						players.get(i).sendMessage(ChatColor.AQUA + "[PMGO-L] " + ChatColor.WHITE + "" + ChatColor.RED + "레드 팀" + ChatColor.WHITE + " 이 되셨습니다.");
					...
					if(minigame.getBlueTeam().size() < minigame.getRedTeam().size() - 1){
						...
						team = 1;
						players.get(i).sendMessage(ChatColor.AQUA + "[PMGO-L] " + ChatColor.WHITE + "" + ChatColor.BLUE + "블루 팀" + ChatColor.WHITE + " 이 되셨습니다.");
						mdata.sendMinigameMessage(minigame, players.get(i).getName() + " 님은 " + ChatColor.BLUE + "블루 팀" + ChatColor.WHITE + " 이 되셨습니다.", null, players.get(i));
					}
				}
				else{
...
//				pdata.addPlayerScore(attacker);
				...
				if(team != ateam){
					...
					if(end){
						mdata.sendMinigameMessage(mgm, attacker.getName() + " 님이 " + ply.getName() + "님에게 마지막 킬을 당하셨습니다!", null, null);
						if(ateam == 1){
							if(mgm.getMaxScore() != 0 && mgm.getBlueTeamScore() >= mgm.getMaxScorePerPlayer(mgm.getPlayers().size())){
			...
			if(mgm.getScoreType().equals("kills")){
				if(pteam == 1){
					if(mgm.getRedTeam().size() < mgm.getBlueTeam().size() - 1){
						...
						ply.sendMessage(ChatColor.AQUA + "[PMGO-L] " + ChatColor.WHITE + "당신의 팀이 " + ChatColor.RED + "레드 팀" + ChatColor.WHITE + " 으로 변경되었습니다!");
						mdata.sendMinigameMessage(mgm, ChatColor.AQUA + "[PMGO-L] " + ChatColor.WHITE + ply.getName() + " 님의 팀이 " + ChatColor.RED + "레드 팀" + ChatColor.WHITE + " 으로 변경되었습니다!", "info", ply);
					}
				}
				else{
					if(mgm.getBlueTeam().size() < mgm.getRedTeam().size()  - 1){
						TeamDMMinigame.switchTeam(mgm, ply);
						ply.sendMessage(ChatColor.AQUA + "[PMGO-L] " + ChatColor.WHITE + "당신의 팀이 " + ChatColor.BLUE + "블루 팀" + ChatColor.WHITE + " 으로 변경되었습니다!");
						mdata.sendMinigameMessage(mgm, ChatColor.AQUA + "[PMGO-L] " + ChatColor.WHITE + ply.getName() + " 님의 팀이 " + ChatColor.BLUE + "블루 팀" + ChatColor.WHITE + " 으로 변경되었습니다!", "info", ply);
					}
				}
			}
		}
