	...
	public void balanceTeam(List<MinigamePlayer> players, Minigame minigame) {
		for(int i = 0; i < players.size(); i++){
			...
			if(!minigame.getType().equals("teamdm")){
				...
				ply.sendMessage(ChatColor.RED + "[PMGO-L] " + ChatColor.WHITE + "감염 게임은 팀 데스매치에서만 할 수 있습니다!");
			}
			else{
				int team = -1;
				...
				if(team == 1){
					if(minigame.getRedTeam().size() < Math.ceil(players.size() * 0.18)){
						...
						team = 0;
						players.get(i).sendMessage(ChatColor.AQUA + "[PMGO-L] " + ChatColor.WHITE + "" + ChatColor.RED + "감염되셨습니다!");
						mdata.sendMinigameMessage(minigame, players.get(i).getName() + " 님이 " + ChatColor.RED + "감염되셨습니다!", null, players.get(i));
					}
				}
				else if(team == -1){
					if(minigame.getRedTeam().size() < Math.ceil(players.size() * 0.18)){
...
//					pdata.setPlayerScore(event.getEntity(), 0);
					...
					if(mgm.getLives() != player.getDeaths()){
						mdata.sendMinigameMessage(mgm, event.getEntity().getName() + " 님이 " + ChatColor.RED + "감염되셨습니다!", "error", null);
					}
