...
//				pdata.storePlayerInventory(player, items, armour, health, food, saturation);
			...
			log.log(Level.SEVERE, "저장된 인벤토리를 로드하지 못했습니다!");
			e.printStackTrace();
		}
		...
		try{
			...
			try{
				...
				log.log(Level.SEVERE, "config.yml 를 저장할 수 없습니다!");
				e.printStackTrace();
			}
		}
		catch(Exception e){
		...
		if(!setupEconomy()){
	        getLogger().info("Vault 플러그인이 없습니다! 아이템만 보상으로 줄 수 있습니다.");
		 }
		...
		if(cal.get(Calendar.DAY_OF_MONTH) == 21 && cal.get(Calendar.MONTH) == 8 ||
			...
			getLogger().info(ChatColor.GREEN.name() + "파티 모드 활성화됨!");
