...
public class FlagSign implements MinigameSign {
	...
	public boolean signCreate(SignChangeEvent event) {
		...
		if(event.getLine(2).equalsIgnoreCase("레드")){
		...
		else if(event.getLine(2).equalsIgnoreCase("캡쳐") && !event.getLine(3).isEmpty()){
			event.setLine(2, ChatColor.GREEN + "캡쳐");
			if(event.getLine(3).equalsIgnoreCase("레드")){
				...
				event.getPlayer().sendMessage(ChatColor.RED + "[PMGO-L] " + ChatColor.WHITE + "표지판 양식이 알맞지 않습니다!" +
						" 가능한 값은 레드,블루,기본입니다.");
				return false;
			}
		}
