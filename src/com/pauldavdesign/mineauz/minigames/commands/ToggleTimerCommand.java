			...
			if(mgm != null){
				if(mgm.getMpTimer() != null){
					if(mgm.getMpTimer().isPaused()){
						...
						mgm.getMpTimer().pauseTimer(sender.getName() + " 님이 타이머를 중지시켰습니다.");
						sender.sendMessage(ChatColor.GRAY + "" + mgm.getName() + " 미니게임의 타이머를 중지시켰습니다. (남은 시간" + mgm.getMpTimer().getPlayerWaitTimeLeft() + "초)");
					}
				}
				else{
					sender.sendMessage(ChatColor.RED + "그 미니게임은 실행된 타이머가 없습니다!");
				}
			}
			else{
				sender.sendMessage(ChatColor.RED + "" + args[0] + " 미니게임은 존재하지 않습니다!");
			}
			return true;
		}
		return false;
