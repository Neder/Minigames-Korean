		...
		taskID = Bukkit.getScheduler().scheduleSyncDelayedTask(Minigames.plugin, new Runnable() {
			...
			public void run() {
				...
				for(MinigamePlayer pl : minigame.getPlayers()){
					if(getTeam() == 0){
						pl.sendMessage(ChatColor.AQUA + "[PMGO-L] " + ChatColor.RED + "레드팀의" + ChatColor.WHITE + " 깃발이 기지로 돌아갔습니다!");
					}else if(getTeam() == 1){
						pl.sendMessage(ChatColor.AQUA + "[PMGO-L] " + ChatColor.BLUE + "블루팀의" + ChatColor.WHITE + " 깃발이 기지로 돌아갔습니다!");
					}
					else{
						pl.sendMessage(ChatColor.AQUA + "[PMGO-L] " + ChatColor.GRAY + "일반" + ChatColor.WHITE + " 깃발이 기지로 돌아갔습니다!");
					}
				}
				taskID = -1;
			}
