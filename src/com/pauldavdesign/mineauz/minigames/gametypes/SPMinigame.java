...
//			pdata.addPlayerMinigame(player, mgm);
			...
			player.sendMessage(ChatColor.AQUA + "[PMGO-L] " + ChatColor.WHITE + 
					"싱글 플레이 미니게임을 시작하셨습니다. /quit 으로 나가실 수 있습니다.");
			
			//pdata.setPlayerCheckpoints(player, startpos);
			player.setCheckpoint(startpos);
			
			if(mgm.getLives() > 0){
				player.sendMessage(ChatColor.AQUA + "[PMGO-L] " + ChatColor.WHITE + "남은 목숨: " + mgm.getLives());
			}
			...
			player.sendMessage(ChatColor.RED + "[PMGO-L] " + ChatColor.WHITE + "이 미니게임은 아직 준비되지 않았습니다!(패배자 이동 지점 없음)");
		}
		else if(!mgm.isEnabled()){
			player.sendMessage(ChatColor.RED + "[PMGO-L] " + ChatColor.WHITE + "이 미니게임은 아직 준비되지 않았습니다!");
		}
		return false;
	}
	...
	public void endMinigame(MinigamePlayer player, Minigame mgm){
		...
		if(plugin.getConfig().getBoolean("singleplayer.broadcastcompletion")){
			plugin.getServer().broadcastMessage(ChatColor.GREEN + "[PMGO-L] " + ChatColor.WHITE + player.getName() + " 님이 " + mgm.getName() + "미니게임을 클리어하였습니다.");
		}
