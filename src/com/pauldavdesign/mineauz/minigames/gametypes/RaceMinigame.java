	...
	public void endMinigame(MinigamePlayer player, Minigame mgm) {
		if(mgm.getMpBets() != null){
			if(mgm.getMpBets().hasBets()){
				...
				player.sendMessage(ChatColor.AQUA + "[PMGO-L] " + ChatColor.WHITE + "당신이 이겨" + mgm.getMpBets().claimMoneyBets() + "원을 받았습니다!");
				mgm.setMpBets(null);
			}
		}
			...
			for(int i = 0; i < players.size(); i++){
				if(players.get(i) instanceof MinigamePlayer){
					...
					if(!p.getName().equals(player.getName())){
						p.sendMessage(ChatColor.RED + "[PMGO-L] " + ChatColor.WHITE + "당신은 졌습니다! 안타깝네요..");
